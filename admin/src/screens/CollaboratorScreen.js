...
import { COLLABORATOR_CREATE_RESET } from "../constants/collaboratorConstants";
const CollaboratorScreen = ({ history, match }) => {
  ...
  const { loading, error, collaborator } = collaboratorList;
  console.log(collaborator);

  const collaboratorDelete = useSelector((state) => state.collaboratorDelete);
  const {
  ...
  const collaboratorCreate = useSelector((state) => state.collaboratorCreate);
  const {
  ...
  useEffect(() => {
    dispatch({ type: COLLABORATOR_CREATE_RESET });
  ...
  }, [history, successDelete, successCreate, createdCollaborator, userInfo]);
  ...
  const createCollaboratorHandler = () => {
            ...
            <div className="flex flex-wrap mt-5 -m-4">
              ...
              {collaborator &&
                collaborator.map((collaborator) => (
                  <div key={collaborator._id} className="p-4 md:w-1/3 w-full">
                    <div className="h-full bg-white  hover:shadow-md space-y-5 shadow border border-gray-100 p-5 rounded">
                      ...
                      <button
                        onClick={() => deleteHandler(collaborator._id)}
