...
import {
  listSuperstarDonarDetails,
  updateSuperstarDonar,
} from "../actions/superstarDonarActions";
import { SUPERSTARDONAR_UPDATE_RESET } from "../constants/superstarDonarConstants";
import { TrashIcon } from "@heroicons/react/outline";
const SuperstarDonarScreen = ({ history, match }) => {
  ...
  const superstarDonarId = match.params.id;

  const superstarDonarDetails = useSelector(
    (state) => state.superstarDonarDetails
  );
  const { loading, error, superstarDonar } = superstarDonarDetails;

  // console.log(superstarDonar);

  const superstarDonarUpdate = useSelector(
    (state) => state.superstarDonarUpdate
  );
  const {
    loading: loadingUpdate,
    error: errorUpdate,
    success: successUpdate,
  } = superstarDonarUpdate;
  ...
  useEffect(() => {
    if (successUpdate) {
      dispatch({ type: SUPERSTARDONAR_UPDATE_RESET });
      history.push("/superstarDonor");
    } else {
      if (!superstarDonar.name || superstarDonar._id !== superstarDonarId) {
        dispatch(listSuperstarDonarDetails(superstarDonarId));
      } else {
        setName(superstarDonar.name);
        setDesignation(superstarDonar.designation);
        setDescription(superstarDonar.description);
        setTwitterLink(superstarDonar.twitterLink);
        setFbLink(superstarDonar.fbLink);
        setImage(superstarDonar.image);
        // console.log(description);
      }
    }
    // console.log(tags);
    // console.log(editorJsInstance);
  }, [dispatch, history, superstarDonarId, superstarDonar, successUpdate]);
            ...
            <div className="flex flex-wrap mt-5 -m-4">
              {superstarDonars &&
                superstarDonars.map((superstarDonar) => (
                  <div key={superstarDonar._id} className="p-4 md:w-1/2 w-full">
                    <div className="h-full bg-white  hover:shadow-md shadow border border-gray-100 p-5 rounded">
                      ...
                      <div className="my-3">
                        <h1 className="text-primary-500 font-bold text-base">
                          {superstarDonar.heading}
                        </h1>
                        <p className="text-base">{superstarDonar.description}</p>
                      </div>
